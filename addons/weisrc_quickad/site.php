...
class weisrc_quickadModuleSite extends WeModuleSite {
	...
	public $actions_titles = array('fans' => '用户管理', 'order' => '交易记录', 'help' => '使用教程', 'style' => '样式设置', 'ad' => '广告设置', 'setting' => '系统设置');
	private $version = '';
	...
	public function doMobileSumbitInfo() {
		...
		$data = array('title1' => trim($_GPC['title1']), 'title2' => trim($_GPC['title2']), 'mobile' => trim($_GPC['mobile']), 'ad_url' => trim($_GPC['ad_url']));
	...
	public function doWebad() {
		...
		$setting = pdo_fetch('SELECT * FROM ' . tablename($this->table_setting) . ' WHERE uniacid = :uniacid LIMIT 1', array(':uniacid' => $uniacid));
		if (checksubmit('submit')) {
			$data = array('uniacid' => $_W['uniacid'], 'mobile' => trim($_GPC['mobile']), 'title1' => trim($_GPC['title1']), 'title2' => trim($_GPC['title2']), 'qrcode' => trim($_GPC['qrcode']), 'ad' => trim($_GPC['ad']), 'ad_url' => trim($_GPC['ad_url']));
			if (empty($setting)) {
				pdo_insert($this->table_setting, $data);
			} else {
				unset($data['dateline']);
	...
	public function doWebfans() {
		...
		if ($operation == 'display') {
			...
			if (checksubmit()) {
				$data = array('uniacid' => $uniacid, 'nickname' => trim($_GPC['nickname']), 'username' => trim($_GPC['username']), 'mobile' => trim($_GPC['mobile']), 'title1' => trim($_GPC['title1']), 'title2' => trim($_GPC['title2']), 'headimgurl' => trim($_GPC['headimgurl']), 'qrcode' => trim($_GPC['qrcode']), 'ad' => trim($_GPC['ad']), 'ad_url' => trim($_GPC['ad_url']), 'status' => intval($_GPC['status']), 'dateline' => TIMESTAMP);
				if (empty($item)) {
