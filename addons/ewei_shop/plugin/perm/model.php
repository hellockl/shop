...
if (!class_exists('PermModel')){
	class PermModel extends PluginModel
	{
        ...
        private function check($permtype = '')
		{
            ...
            $user = pdo_fetch('select u.status as userstatus,r.status as rolestatus,u.perms as userperms,r.perms as roleperms,u.roleid from ' . tablename('ewei_shop_perm_user') . ' u ' . ' left join ' . tablename('ewei_shop_perm_role') . ' r on u.roleid = r.id ' . ' where uid=:uid limit 1 ', array(':uid' => $uid));
		...
		function check_plugin($pluginname = '')
		{
			...
			if (!empty($ac_perm)) {
                ...
                load() -> model('account');
                $founder = uni_owned($_W['founder']);
                if(in_array($_W['uniacid'], array_keys($founder))){
                    $allow = true;
                }else{
                    $allow = false;
                }
