...
if ($operation == "display") {
    ...
    //判断该帐号的权限
    if(p('supplier')){
        $roleid = pdo_fetchcolumn('select roleid from' . tablename('ewei_shop_perm_user') . ' where uid='.$_W['uid'].' and uniacid=' . $_W['uniacid']);
        if($roleid == 0){
            $perm_role = 0;
        }else{
            $perm_role = pdo_fetchcolumn('select status1 from' . tablename('ewei_shop_perm_role') . ' where id=' . $roleid);
        }
    }
    //END
    ...
    if(p('supplier')){
        if($perm_role == 1){
            $sql = "select o.* , a.realname as arealname,a.mobile as amobile,a.province as aprofince ,a.city as acity , a.area as aarea,a.address as aaddress, d.dispatchname,r.goodsid,r.supplier_uid,m.nickname,m.id as mid from " . tablename('ewei_shop_order') . " o" . " 
             join 
            ( select ro.id as orderid,g.supplier_uid,g.goodsid,rr.status from " . tablename('ewei_shop_order') . " ro left join " . tablename('ewei_shop_order_refund') . " rr  on rr.orderid =ro.id left join " . tablename('ewei_shop_order_goods') . " g on ro.id = g.orderid where g.supplier_uid=".$_W['uid']." ) r 
            on r.orderid= o.id" . " left join " . tablename('ewei_shop_member') . " m on m.openid=o.openid and m.uniacid =  o.uniacid " . " left join " . tablename('ewei_shop_member_address') . " a on a.id=o.addressid " . ' left join ' . tablename('ewei_shop_saler') . ' s on s.openid = o.verifyopenid and s.uniacid=o.uniacid' . ' left join ' . tablename('ewei_shop_member') . ' sm on sm.openid = o.verifyopenid and sm.uniacid=o.uniacid' . " left join " . tablename('ewei_shop_dispatch') . " d on d.id = o.dispatchid " . "where $condition $statuscondition GROUP BY o.ordersn ORDER BY o.createtime DESC ";	
            ...
            on o.id=og.orderid 
            left join 
            ' . tablename('ewei_shop_goods_option') . ' go 
            on go.id=og.optionid 
            left join 
            ' . tablename('ewei_shop_goods') . ' g 
            on g.id=og.goodsid where og.supplier_uid=:supplier_uid and og.supplier_apply_status!=1 and o.status=3 and og.uniacid=:uniacid', array(':supplier_uid' => $_W['uid'], ':uniacid' => $_W['uniacid']))			;	
            $openid = pdo_fetchcolumn('select openid from ' . tablename('ewei_shop_perm_user') . ' where uid=:uid and uniacid=:uniacid',array(':uid' => $_W['uid'],':uniacid'=> $_W['uniacid']));
            if(empty($openid)){
            ...
            if(!empty($applytype)){
                $mygoodsid = pdo_fetchall('select id from ' . tablename('ewei_shop_order_goods') . 'where supplier_uid=:supplier_uid and supplier_apply_status = 0',array(
                        ':supplier_uid' => $_W['uid']
                    ));
                if(empty($mygoodsid)){
                    message("没有可提现的订单金额");
                ...
                pdo_insert('ewei_shop_supplier_apply',$data);
                
                foreach ($mygoodsid as $ids) {
                    $arr = array(
                        'supplier_apply_status' => 1
                        );
                    pdo_update('ewei_shop_order_goods', $arr, array(
                        'id' => $ids['id']
                        ));
                }
            ...
            $sql = "select o.* , a.realname as arealname,a.mobile as amobile,a.province as aprofince ,a.city as acity , a.area as aarea,a.address as aaddress, d.dispatchname,m.nickname,m.id as mid from " . tablename('ewei_shop_order') . " o" . " left join ( select rr.id,rr.orderid,g.supplier_uid,rr.status from " . tablename('ewei_shop_order_refund') . " rr left join " . tablename('ewei_shop_order') . " ro on rr.orderid =ro.id left join " . tablename('ewei_shop_order_goods') . " g on ro.id = g.orderid order by rr.id desc limit 1) r on r.orderid= o.id" . " left join " . tablename('ewei_shop_member') . " m on m.openid=o.openid and m.uniacid =  o.uniacid " . " left join " . tablename('ewei_shop_member_address') . " a on a.id=o.addressid " . ' left join ' . tablename('ewei_shop_saler') . ' s on s.openid = o.verifyopenid and s.uniacid=o.uniacid' . ' left join ' . tablename('ewei_shop_member') . ' sm on sm.openid = o.verifyopenid and sm.uniacid=o.uniacid' . " left join " . tablename('ewei_shop_dispatch') . " d on d.id = o.dispatchid " . " where $condition $statuscondition ORDER BY o.createtime DESC,o.status DESC  ";
        }
    }else{
    ...
    if(p('supplier')){
        if($perm_role == 1){
            /*$totalmoney = pdo_fetchcolumn('select ifnull(sum(o.price),0) from ' . tablename('ewei_shop_order_goods') . ' og left join ' . tablename('ewei_shop_order') . ' o on o.id=og.orderid left join ' . tablename('ewei_shop_goods') . ' g on g.id=og.goodsid where og.supplier_uid=:supplier_uid and og.uniacid=:uniacid',array(
                    ':supplier_uid' => $_W['uid'],
                    ':uniacid' => $_W['uniacid']
                ));*/
			$orderid = pdo_fetchall("SELECT distinct orderid FROM ".tablename('ewei_shop_order_goods')." WHERE supplier_uid = :supplier_uid ", array(':supplier_uid' => $_W['uid']));	
			if($orderid){
				$orderid = array_multi2single($orderid);
				$orderids= implode(',',$orderid); 	
				$totalmoney = pdo_fetchcolumn("SELECT SUM(price) FROM ims_ewei_shop_order WHERE id in ($orderids)");
			}else{
				$totalmoney = pdo_fetchcolumn('select ifnull(sum(o.price),0) from ' . tablename('ewei_shop_order_goods') . ' og left join ' . tablename('ewei_shop_order') . ' o on o.id=og.orderid left join ' . tablename('ewei_shop_goods') . ' g on g.id=og.goodsid where og.supplier_uid=:supplier_uid and og.uniacid=:uniacid',array(
                    ':supplier_uid' => $_W['uid'],
                    ':uniacid' => $_W['uniacid']
            ...
            $totals = array();
            $totals['all'] = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ewei_shop_order') . ' where supplier_uid=' . $_W['uid']);
            $totals['status_1'] = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ewei_shop_order') . ' where status=-1 and supplier_uid=' . $_W['uid']);
            $totals['status0'] = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ewei_shop_order') . ' where status=0 and supplier_uid=' . $_W['uid']);
            $totals['status1'] = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ewei_shop_order') . ' where status=1 and supplier_uid=' . $_W['uid']);
            $totals['status2'] = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ewei_shop_order') . ' where status=2 and supplier_uid=' . $_W['uid']);
            $totals['status3'] = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ewei_shop_order') . ' where status=3 and supplier_uid=' . $_W['uid']);
            $totals['status4'] = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ewei_shop_order') . ' o left join ' .tablename('ewei_shop_order_refund') . ' ro on ro.orderid=o.id where ro.status=0 and o.supplier_uid=' . $_W['uid']);
            $totals['status5'] = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ewei_shop_order') . ' o left join ' .tablename('ewei_shop_order_refund') . ' ro on ro.orderid=o.id where ro.status=1 and o.supplier_uid=' . $_W['uid']);
        }else{
    ...
    if(p('supplier')){
        if($perm_role == 1){
            $supplierapply = pdo_fetchall('select u.uid,p.realname,p.mobile,p.banknumber,p.accountname,p.accountbank,a.applysn,a.apply_money,a.apply_time,a.type,a.finish_time,a.status from ' . tablename('ewei_shop_supplier_apply') . ' a ' . ' left join' . tablename('ewei_shop_perm_user') . ' p on p.uid=a.uid ' . 'left join' . tablename('users') . ' u on a.uid=u.uid where u.uid=' . $_W['uid']);
	...
	/* 供应商增加$cond条件 */
    $total = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename("ewei_shop_order") . " o " . " left join ( select rr.id,rr.orderid,rr.status from " . tablename("ewei_shop_order_refund") . " rr left join " . tablename("ewei_shop_order") . " ro on rr.orderid =ro.id order by rr.id desc limit 1) r on r.orderid= o.id" . " left join " . tablename("ewei_shop_member") . " m on m.openid=o.openid  and m.uniacid =  o.uniacid" . " left join " . tablename("ewei_shop_member_address") . " a on o.addressid = a.id " . " left join " . tablename("ewei_shop_member") . " sm on sm.openid = o.verifyopenid and sm.uniacid=o.uniacid" . " left join " . tablename("ewei_shop_saler") . " s on s.openid = o.verifyopenid and s.uniacid=o.uniacid" . " WHERE $condition $statuscondition " . $cond , $paras);
