...
if ($_W["isajax"]) {
    if ($operation == "cancel") {
        ...
        $order = pdo_fetch("select id,ordersn,openid,status,deductcredit,deductprice,couponid from " . tablename("ewei_shop_order") . " where id=:id and uniacid=:uniacid and openid=:openid limit 1", array(
            ":id" => $orderid,
            ":uniacid" => $uniacid,
            ":openid" => $openid
        ));
        ...
        $orderid = intval($_GPC["orderid"]);
        $order = pdo_fetch("select id,status,openid,couponid from " . tablename("ewei_shop_order") . " where id=:id and uniacid=:uniacid and openid=:openid limit 1", array(
            ":id" => $orderid,
            ":uniacid" => $uniacid,
            ":openid" => $openid
        ));
