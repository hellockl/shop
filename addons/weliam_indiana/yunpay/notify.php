...
$params = array();
$params[':tid'] = trim($_REQUEST['out_order_no']);
...
if(is_array($setting['payment'])) {
	...
	if(!empty($yunpay)) {
		////////////////////////////////////////////////////////////////
		//计算得出通知验证结果
	$yunNotify = md5VerifyShan($_REQUEST['out_order_no'],$_REQUEST['total_fee'],$_REQUEST['trade_status'],$_REQUEST['sign'],$yunpay['key'],$yunpay['partner']);
	//echo $yunNotify;exit;
	if($yunNotify && $_REQUEST['trade_status']=='TRADE_SUCCESS') {//验证成功
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	//商户订单号
	...
	/*********************检测支付是否异常结束**********************/
		/////////////////////////////////////////////////////////////////
			if(!empty($get)){
				...
				$sql = 'SELECT * FROM ' . tablename('core_paylog') . ' WHERE `tid`=:tid';
				$params = array();
				$params[':tid'] = $_REQUEST['out_order_no'];
